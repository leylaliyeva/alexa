...
app = Flask(__name__)
REAL_SERVICE_URL = 'http://localhost:5000/'
...
def proxy(path):
    ...
    if response_time > baseline_median * threshold:
        response_time_alert = f"Alert! Response time {response_time} ms is significantly higher than baseline median {baseline_median} ms - Alexa Service"
    else:
        response_time_alert = f"Response time {response_time} ms is within acceptable range."
